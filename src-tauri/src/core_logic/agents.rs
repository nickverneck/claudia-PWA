[File content truncated: showing lines 1-2000 of 2179 total lines. Use offset/limit parameters to view more.]
use anyhow::Result;
use chrono;
use dirs;
...
pub fn init_database(app: &AppHandle) -> SqliteResult<Connection> {
    ...
    init_database_web(&db_path)
}

pub fn init_database_web(db_path: &std::path::Path) -> SqliteResult<Connection> {
...
pub fn list_agents(db: &AgentDb) -> Result<Vec<Agent>, String> {
...
pub fn delete_agent(db: &AgentDb, id: i64) -> Result<(), String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
...
pub fn get_agent(db: &AgentDb, id: i64) -> Result<Agent, String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
...
pub fn get_agent_run(db: &AgentDb, id: i64) -> Result<AgentRun, String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
...
pub async fn list_agent_runs_with_metrics(
    ...
    let runs = list_agent_runs(db, agent_id)?;
...
pub async fn execute_agent(
    ...
    model: Option<String>,
    db: &AgentDb,
    registry: &process::ProcessRegistryState,
) -> Result<i64, String> {
...
pub fn cleanup_finished_processes(db: &AgentDb) -> Result<Vec<i64>, String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
...
pub fn get_live_session_output(
    registry: &process::ProcessRegistryState,
    run_id: i64,
...
pub fn export_agent(db: &AgentDb, id: i64) -> Result<String, String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
...
pub fn export_agent_to_file(
    ...
    // Get the JSON data
    let json_data = export_agent(db, id)?;
...
pub fn get_claude_binary_path(db: &AgentDb) -> Result<Option<String>, String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
...
pub fn set_claude_binary_path(db: &AgentDb, path: String) -> Result<(), String> {
    let conn = db.0.lock().map_err(|e| e.to_string())?;
...
pub fn list_claude_installations(
    _app: &AppHandle,
) -> Result<Vec<crate::claude_binary::ClaudeInstallation>, String> {
    let installations = claude_binary::discover_claude_installations();
...
pub fn import_agent(db: &AgentDb, json_data: String) -> Result<Agent, String> {
    // Parse the JSON data
